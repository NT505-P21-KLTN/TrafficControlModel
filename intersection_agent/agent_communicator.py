        ...
        try:
            ...
            for state_entry in send_data.get('states', []):
                if state_entry.get('state') is not None:
                    # FIXED: Convert numpy arrays to lists for JSON serialization
                    processed_entry = state_entry.copy()
                    state = processed_entry.get('state')
                    if state is not None:
                        if hasattr(state, 'tolist'):  # numpy array
                            processed_entry['state'] = state.tolist()
                        elif isinstance(state, (list, tuple)):
                            processed_entry['state'] = list(state)
                        # else: keep as is (already JSON serializable)

                    # Also check traffic_data for numpy arrays
                    traffic_data = processed_entry.get('traffic_data', {})
                    if traffic_data:
                        processed_traffic_data = {}
                        for key, value in traffic_data.items():
                            if hasattr(value, 'tolist'):  # numpy array
                                processed_traffic_data[key] = value.tolist()
                            elif isinstance(value, dict):
                                # Handle nested dictionaries (like avg_speed, incoming_vehicles)
                                processed_nested = {}
                                for nested_key, nested_value in value.items():
                                    if hasattr(nested_value, 'tolist'):
                                        processed_nested[nested_key] = nested_value.tolist()
                                    else:
                                        processed_nested[nested_key] = nested_value
                                processed_traffic_data[key] = processed_nested
                            else:
                                processed_traffic_data[key] = value
                        processed_entry['traffic_data'] = processed_traffic_data

                    valid_states.append(processed_entry)

            # Replace states with filtered and processed valid states
